...
#'
#' @param PSOCK.names `name` of [parallel::makePSOCKcluster]. If `NULL` is provided,
#' parallel computing feature will be turned off.
#'
#' @returns None.
#' @export
ecParallel <- function(PSOCK.names){
  ...
  if (is.null(PSOCK.names)){
    ecOptions(parallel = F)
  } else{
    ecOptions(parallel = T, PSOCK.names = PSOCK.names)
  }
...
#'
#' * `[[]]` double bracket subsetting, which should return an object in the list.
#' Integer subsetting must be supported.
...
#' # Arguments supplied
#'
#' `ecParallelFunc()` will take arbitrary arguments supplied as list-like objects
...
#'
#' [ecOptions()] describes options of this package including parallel computation.
#' If `parallel == FALSE`, `ecParallelFunc()` will be running the combinations in
...
ecParallelFunc <- function(func, ...){
  ...
  stopifnot(all(
    sapply(args, is.list.like)
  ))
  #   Argument list lengths must be the same or 1
  arg.lens <- sapply(args, length)
  ...
  if (par){
    ...
    pMApply <- function(...) parallel::clusterMap(cl = cl, fun = func,
                                                  .scheduling = "static", ...)
  } else{
