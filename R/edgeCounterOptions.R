...
#       `parallel` - BOOLEAN
#       `PSOCK.names` - integer or character vector, see `parallel::makeCluster`
...
#' values are functions for validating the option. Validation functions must take
#' in a single argument and return Boolean.
...
#' for parallel computation. A 'diligent' approach for initialization and clean-up
#' of the PSOCK cluster is used, meaning that the PSOCK cluster is initialized
#' only when functions supporting parallel computation is called, and the cluster
#' is stopped whenever the called function exits.
#' @export
ecOptions <- function(...){
  ...
  if (length(args) != 0){
    ...
    stopifnot(all(sapply(names(args), function(n) supported[[n]](args[[n]]))))
  }
